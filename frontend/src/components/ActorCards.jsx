...
import { Search, Calendar, Play,List, User, ChevronDown, ChevronUp, CheckCircle, Clock, AlertCircle, Eye, ExternalLink, FileText, Globe } from "lucide-react";
import { useState } from "react";
import { useStore } from "../store/store.js";
import ActorInputForm from "./ActorInputForm.jsx";
...
const ActorCards = ({ actors }) => {
  // console.log("Actors in ActorCards:", actors);
  const { fetchInputParams, inputParams, loading, getRunResult, result, getLatestResult, scrapedData } = useStore();
  const [showForm, setShowForm] = useState(false);
  const [selectedActor, setSelectedActor] = useState(null);
  const [showResults, setShowResults] = useState({});
  const [completedRuns, setCompletedRuns] = useState({});
  ...
  const renderLatestResult = (actorId) => {
    ...
    console.log("Rendering latest result for actorId:", actorId, "Result:", result, "ScrapedData:", scrapedData); // Debug log
        ...
        <div className="space-y-4">
          ...
          {scrapedData && (
            <div className="border-t border-gray-200 pt-4">
              ...
              {scrapedData.url && (
                <div className="mb-3">
                  <span className="text-sm font-medium text-gray-700 mr-2">URL:</span>
                  <a 
              ...
              {scrapedData.metadata?.title && (
                <div className="mb-3">
                  <span className="text-sm font-medium text-gray-700 mr-2">Title:</span>
                  <span className="text-sm text-gray-900">{scrapedData.metadata.title}</span>
                </div>
              )}
              ...
              {scrapedData.metadata?.description && (
                <div className="mb-3">
                  ...
                  <span className="text-sm text-gray-600">{scrapedData.metadata.description}</span>
                </div>
              )}
              ...
              {/* Markdown Content */}
              {scrapedData.markdown && scrapedData.markdown !== scrapedData.text && (
                <div className="mb-3">
                  <span className="text-sm font-medium text-gray-700 block mb-2">Markdown:</span>
                  <div className="bg-white p-3 rounded border border-gray-200 max-h-32 overflow-y-auto">
                    <pre className="text-sm text-gray-800 whitespace-pre-wrap">
              ...
              {scrapedData.crawl && (
                <div className="grid grid-cols-2 gap-4 mt-3 pt-3 border-t border-gray-100">
                  {scrapedData.crawl.depth !== undefined && (
                    <div>
                      <span className="text-xs font-medium text-gray-600">Depth:</span>
                      <span className="text-xs text-gray-800 ml-1">{scrapedData.crawl.depth}</span>
                    </div>
                  )}
                  {scrapedData.crawl.httpStatusCode && (
                    <div>
                      <span className="text-xs font-medium text-gray-600">HTTP Status:</span>
                      <span className={`text-xs ml-1 px-1 rounded ${
                        scrapedData.crawl.httpStatusCode === 200 ? 'bg-green-100 text-green-800' : 
                  ...
                  {scrapedData.crawl.loadedTime && (
                    <div className="col-span-2">
                      <span className="text-xs font-medium text-gray-600">Loaded:</span>
                      <span className="text-xs text-gray-800 ml-1">
                        {new Date(scrapedData.crawl.loadedTime).toLocaleString()}
                      </span>
                    </div>
                  )}
                </div>
          ...
          {result.output && (
            <div className="border-t border-gray-200 pt-3">
              <span className="text-sm font-medium text-gray-700 block mb-2">Run Output:</span>
              <div className="bg-white p-3 rounded border border-gray-200 max-h-64 overflow-y-auto">
                <pre className="text-sm text-gray-800 whitespace-pre-wrap">
                  {typeof result.output === 'string' ? result.output : JSON.stringify(result.output, null, 2)}
                </pre>
